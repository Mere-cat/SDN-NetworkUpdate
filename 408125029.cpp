...
// Version: 3
// 
// 上一版：真的會把更新後的SPT算出來，但就只是這樣，不會影響輸出
// 更新： 把新SPT的規則直接塞給switch當作更新，不做round by round 更新
////////////////////////////////////////////////////
...
int numOfSwitch, SrcID, DstID;

// To initialize the newGraph
bool InitGraph = 0;

// The original graph
vector <vector<int>> origGraph;

// routing table recording
map<unsigned int, unsigned int> origRT;
map<unsigned int, unsigned int> updRT;
...
class SDN_switch: public node {
        ...
        std::map<unsigned int, unsigned int> routingTable;
...
class SDN_controller: public node {
        ...
        // Record "ack" returned by switch
        std::map<unsigned int, unsigned int>newAck;
        std::map<unsigned int, unsigned int>updAck;
        std::map<unsigned int, unsigned int>delAck;
        std::map<unsigned int, unsigned int>statAck;

        // Compute the round
        // Round.first -> the dst
        // Round.second -> update round table (key: #round, value: nodes in vector)
        map<unsigned int, map<int, vector<int>>>Round;

        // Compute the path
        vector<vector<int>> newGraph;
        vector<vector<int>> updGraph;
    ...
    public:
        ...
        class SDN_controller_generator : public node_generator{
                ...
                SDN_controller_generator() { 
                    /*cout << "SDN_controller registered" << endl;*/ register_node_type(&sample);
                }
            protected:
        ...
        // To generate SPT with old/new-weighted graph
        class Tree {
            public:
            int numOfNode;
            int *distTable; // distance table, to record the distance from destination to all nodes
            int *prevNode;  // the previous node from destination to all nodes = the next node from switches to destination
            bool *sptSet;   // record the node is gone through or not
            int **tempTree;

            Tree(int numOfNode); // init distTable, prevNode and sptSet
            int minDist();
            void dijkstra(int dest, vector<vector<int>> adjMatrix);
            void printDistTable();
        };
};
...
class SDN_stat_payload : public payload {
        ...
        vector<pair<unsigned int, unsigned int>> LinkWeight;
        int aaa;
    ...
    public:
        ...
        void setLinkWeight(vector<pair<unsigned int, unsigned int>> _LinkWeight){ LinkWeight = _LinkWeight; }
        vector<pair<unsigned int, unsigned int>> getLinkWeight(){ return LinkWeight; }
...
void stat_packet_event (unsigned int src_id, unsigned int dst_id, vector<pair<unsigned int, unsigned int>> LinkWeight, unsigned int t = event::getCurTime(), string msg = "default"){
...
void SDN_switch::recv_handler (packet *p){
    ...
    if (p->type() == "SDN_data_packet" ) { // the switch receives a packet from the other switch
        ...
        SDN_data_packet * p5 = nullptr;
        p5 = dynamic_cast<SDN_data_packet*> (p);

        int dst = p5->getHeader()->getDstID();
        if(routingTable.find(dst) == routingTable.end()) return;

        p5->getHeader()->setPreID ( getNodeID() );
        p5->getHeader()->setNexID ( routingTable[dst] );
        p5->getHeader()->setDstID ( dst );
        //cout << getNodeID() << " " << routingTable[dst] << endl;

        send_handler (p5);
    } else if (p->type() == "SDN_ctrl_new_packet") { 
    // the switch receives a packet from the controller -> write the new rule to its RoutingTable
        SDN_ctrl_new_packet *p1 = nullptr;
        p1 = dynamic_cast<SDN_ctrl_new_packet*> (p);
        SDN_ctrl_new_payload *l1 = nullptr;
        l1 = dynamic_cast<SDN_ctrl_new_payload*> (p1->getPayload());

        // set the routing table
        unsigned mat = l1->getMatID();
        unsigned act = l1->getActID();
        routingTable[mat] = act;

        // retrun "ack" to controller
        p1->getHeader()->setPreID(node::getNodeID());    // PreID is itself
        p1->getHeader()->setNexID(node::getNodeNum()-1); // NextID is controller

        p1->getHeader()->setSrcID(node::getNodeID());    // SrcID is itself
        p1->getHeader()->setDstID(node::getNodeNum()-1); // DstID to controller

        send_handler(p1);
    } else if (p->type() == "SDN_ctrl_upd_packet") {
    // update the new rule to its RoutingTable
        SDN_ctrl_upd_packet *p2 = nullptr;
        p2 = dynamic_cast<SDN_ctrl_upd_packet*> (p);

        SDN_ctrl_upd_payload *l2 = nullptr;
        l2 = dynamic_cast<SDN_ctrl_upd_payload*> (p2->getPayload());
        ...
        // retrun "ack" to controller
        p2->getHeader()->setPreID(node::getNodeID());    // PreID is itself
        p2->getHeader()->setNexID(node::getNodeNum()-1); // NextID is controller

        p2->getHeader()->setSrcID(node::getNodeID());    // SrcID is itself
        p2->getHeader()->setDstID(node::getNodeNum()-1); // DstID to controller
        send_handler(p2);

    } else if (p->type() == "SDN_ctrl_del_packet") {
    // delete its RoutingTable
        ...
        routingTable.erase(mat);

        // retrun "ack" to controller
        p3->getHeader()->setPreID(node::getNodeID());    // PreID is itself
        p3->getHeader()->setNexID(node::getNodeNum()-1); // NextID is controller

        p3->getHeader()->setSrcID(node::getNodeID());    // SrcID is itself
        p3->getHeader()->setDstID(node::getNodeNum()-1); // DstID to controller
        send_handler(p3);
        ...
        p4->getHeader()->setPreID(node::getNodeID());    // PreID is itself
        p4->getHeader()->setNexID(node::getNodeNum()-1); // NextID is controller

        p4->getHeader()->setSrcID(node::getNodeID());    // SrcID is itself
        p4->getHeader()->setDstID(node::getNodeNum()-1); // DstID to controller

        //cout << p3->getHeader()->getPreID() << endl;
        //cout << p3->getHeader()->getNexID() << endl;
        send_handler(p4);
    }
...
void SDN_controller::recv_handler (packet *p){
    ...
    if (p->type() == "SDN_ctrl_new_packet") {
        ...
        SDN_ctrl_new_payload *l1 = nullptr;
        l1 = dynamic_cast<SDN_ctrl_new_payload*> (p1->getPayload());
        //cout << p1->getHeader()->getDstID() << " " << p1->getHeader()->getNexID() << " " << p1->getHeader()->getPreID() << " " << p1->getHeader()->getSrcID();
        
        // Only send to switch if src is the controller itself
        newAck[p1->getHeader()->getSrcID()]=1;
        if(p1->getHeader()->getSrcID() == getNodeNum()-1) {
            p1->getHeader()->setNexID(p1->getHeader()->getDstID());
            send_handler(p1);
        }
    } else if (p->type() == "SDN_ctrl_upd_packet") {
        SDN_ctrl_upd_packet *p2 = nullptr;
        ...
        updAck[p2->getHeader()->getSrcID()];
        if(p2->getHeader()->getSrcID() == getNodeNum()-1) {
            p2->getHeader()->setNexID(p2->getHeader()->getDstID());
            send_handler(p2);
        }
        ...
        delAck[p3->getHeader()->getSrcID()];
        if(p3->getHeader()->getSrcID() == getNodeNum()-1) {
        ...
        vector<pair<unsigned, unsigned>> statInfo;
        statInfo = l4->getLinkWeight();

        // Dealing with the stat packet from switch:
        // 1. mark "ack"
        // 2. draw a graph according to the stat_info (LinkWeight) it carried
        statAck[p4->getHeader()->getSrcID()]=1;

        int id = p4->getHeader()->getPreID();
        // Initialize the graph(called newGraph)
        if(InitGraph == 0) {
            for(int i = 0; i < node::getNodeNum() -1; i++) {
                ...
                for(int j = 0; j < 5; j++) {
                    ...
                    updGraph.push_back(tmp);
                }
            }
            InitGraph = 1;
        }

        // draw a graph according to the stat_info
        for(int i = 0; i < statInfo.size(); i++) {
            auto ptr = statInfo[i];
            newGraph[id][ptr.first] = ptr.second;
            newGraph[ptr.first][id] = ptr.second;
        }

        // All the switch send back the stat packet
        // Time to compute minCostPath & update round
        if(statAck.size() == numOfSwitch) {
            // Output the newGraph
            /*for(int i = 0; i < node::getNodeNum()-1; i++) {
                for(int j = 0; j < node::getNodeNum()-1; j++) 
                    cout << newGraph[i][j] << " ";
            ...
            // compute SPT
            SDN_controller::Tree SPT(numOfSwitch);
            SPT.dijkstra(DstID, newGraph);
            for(int j = 0; j < numOfSwitch; j++) {
                // store the information of routing table of node j
                // oldRT is an array of struct whose index is the node's ID, and the struct contains
                // the node's destID and nextID
                //oldRT[j].destID[i] = dstNode[i];
                //oldRT[j].nextID[i] = SPT.prevNode[j];
            }
            for(int i = 0; i < numOfSwitch; i++) {
                if(i == DstID) continue;
                //int j = i;
                int j = SrcID;
                while(SPT.prevNode[j] != DstID) {
                    updRT[j] = SPT.prevNode[j];
                    updGraph[j][SPT.prevNode[j]] = newGraph[j][SPT.prevNode[j]];
                    updGraph[SPT.prevNode[j]][j] = newGraph[j][SPT.prevNode[j]];
                    j = SPT.prevNode[j];
                }
                updRT[j] = SPT.prevNode[j];
                updGraph[j][SPT.prevNode[j]] = newGraph[j][SPT.prevNode[j]];
            ...
            // print updRT
            /*for(int i = 0; i < numOfSwitch; i++) {
                if(updRT.find(i) != updRT.end()) cout << i << " " << updRT[i] << endl;
            }*/
            // print the updGraph
            /*for(int i = 0; i < node::getNodeNum()-1; i++) {
                for(int j = 0; j < node::getNodeNum()-1; j++) 
            ...
            for(int i = 0; i < node::getNodeNum()-1; i++) {
                //for(int j = 0; j < node::getNodeNum()-1; j++) {
                    // new: not in origRT, but in updRT
                    if(origRT.find(i) == origRT.end() && updRT.find(i) != updRT.end()) {
                        //cout << "new: " << i << " " << updRT[i] << endl;
                        ctrl_new_packet_event(node::getNodeNum()-1, i, DstID, updRT[i]);
                    }
                    // update: in origRT & in updRT, but value(act) different
                    else if(origRT.find(i) != origRT.end() && updRT.find(i) != updRT.end() && origRT[i] != updRT[i]) {
                        //cout << "upd: " << i << " " << origRT[i] << "--->" << updRT[i] << endl;
                        ctrl_upd_packet_event(node::getNodeNum()-1, i, DstID, updRT[i]);
                    }
                    // del: in origRT, but not in updRT
                    else if(origRT.find(i) != origRT.end() && updRT.find(i) == updRT.end()) {
                        //cout << "del: " << i << " " << origRT[i] << endl;
                        ctrl_del_packet_event(node::getNodeNum()-1, i, DstID, origRT[i]);
                    }
                //}
            }
        }
...
//////////////////////////////////////////////////////////////////////////////////////
SDN_controller::Tree::Tree(int numOfNode)
{
    distTable = new int[numOfNode];
    sptSet = new bool[numOfNode];
    prevNode = new int[numOfNode];
    /*for (int i = 0; i < numOfNode; i++) {
        distTable.push_back(INF);
        prevNode.push_back(-1);
    }*/
}

int SDN_controller::Tree::minDist()
{
    int min = INF, minDistId = -1;
    for(int i = 0; i < numOfSwitch; i++) {
        if(sptSet[i] == false && distTable[i] < min) {
            min = distTable[i];
            minDistId = i;
        }
    }
...
void SDN_controller::Tree::dijkstra(int dest, vector<vector<int>> adjMatrix)
{
    // Init
    for(int i = 0; i < numOfSwitch; i++) {
        distTable[i] = INF;
    ...
    // Dijkstra
    for(int i = 0; i < numOfSwitch; i++) {
        // Find the shortest distance in the distance table
        // and put it into the SPT set
        int minDistId = minDist();
        sptSet[minDistId] = true;
        //cout << "Select: " << minDistId << endl;
        if(minDistId == SrcID) break; 

        // Update the distance table
        for(int j = 0; j < numOfSwitch; j++) { // j-th node
            //if(minDistId < 0) continue;
            if(!sptSet[j] && adjMatrix[minDistId][j] != 0 && distTable[minDistId] != INF
              && distTable[minDistId] + adjMatrix[minDistId][j] <= distTable[j])
            {
                // If the distance is the same, chose the path to the node
                // with smaller ID
                if(distTable[minDistId] + adjMatrix[minDistId][j] == distTable[j]){
                    if(prevNode[j] > minDistId) prevNode[j] = minDistId;
                    distTable[j] = distTable[minDistId] + adjMatrix[minDistId][j];
                } else {
                    distTable[j] = distTable[minDistId] + adjMatrix[minDistId][j];
                    prevNode[j] = minDistId;
                }
                //printDistTable();
            }
        }
    ...
    for(int i = 0; i < numOfSwitch; i++) {
        if(dest == i) prevNode[i] = -1;
    }
    //printDistTable();
}

void SDN_controller::Tree::printDistTable()
{
    cout <<"Vertex \t Distance Previous Node" << endl;
    for (int i = 0; i < numOfSwitch; i++)
        cout  << i << " \t\t" << distTable[i] << "\t" << prevNode[i] <<endl;
}
...
int main()
{
    ...
    // 1. Read the Input for Install
    int nodeCost, OldPathLen;
    int InsTime, UpdTime, SimDuration;
    int linkID, node1, node2;

    cin >> numOfSwitch >> nodeCost;
    cin >> InsTime >> UpdTime >> SimDuration;
    cin >> SrcID >> DstID >> OldPathLen;

    // generate switches
    for (unsigned int id = 0; id < numOfSwitch; id ++){
        node::node_generator::generate("SDN_switch", id);
    }
    // generate controller
    unsigned int con_id = node::getNodeNum();
    node::node_generator::generate("SDN_controller", con_id); //now node::getNodeNum() will be con_id+1

    // connect every switch to the controller
    for(int i = 0; i < node::getNodeNum(); i++) {
        node::id_to_node(con_id)->add_phy_neighbor(i);
        node::id_to_node(i)->add_phy_neighbor(con_id);
    }

    // Initial the origGraph
    for(int i = 0; i < node::getNodeNum()-1; i++) {
        vector<int> origRow;
        for(int j = 0; j < node::getNodeNum(); j++) {
            origRow.push_back(0);
        }
        origGraph.push_back(origRow);
    }

    // set switches' neighbors and send ctrl_new_packet to controller
    for(int i = 0; i < OldPathLen; i++) {
        ...
        origGraph[node1][node2] = 1;
        origGraph[node2][node1] = 1;
        ctrl_new_packet_event(con_id, node1, DstID, node2, InsTime, "pass_my_OOP_plz");
        origRT[node1] = node2;
    }

    // print origRT
    /*for(int i = 0; i < numOfSwitch; i ++) {
        cout << i << " " << origRT[i] << endl;
    }*/
    ...
    // 2. Read the Input for Update
    int numOfLinks;
    int newW;
    map<unsigned int, vector<pair<unsigned int, unsigned int>>> newWeight;
    pair<unsigned int, unsigned int> tmpLink;

    cin >> numOfLinks;
    cin >> linkID >> node1 >> node2 >> newW;
    // connect nodes with each other using the update info
    for(int i = 1; i < numOfLinks; i++) {
        // add switch to each other's neighbor list
        node::id_to_node(node1)->add_phy_neighbor(node2);
        node::id_to_node(node2)->add_phy_neighbor(node1);

        // prepare stat info, container: map<unsigned int, vector<pair<unsigned int, unsigned int>>>
        // for each map[node], having a vector of pair<neighbor, weight>
        tmpLink = {node2, newW};
        newWeight[node1].push_back(tmpLink);
        cin >> linkID >> node1 >> node2 >> newW;
    }
    node::id_to_node(node1)->add_phy_neighbor(node2);
    node::id_to_node(node2)->add_phy_neighbor(node1);
    ...
    // Send the stat packet to every switch
    for(int i = 0; i < numOfSwitch; i++) {
        vector<pair<unsigned int, unsigned int>> statInfo;
        /*if(newWeight.find(i) == newWeight.end()) {
            //cout << i << endl;
            for(int j = 0; j < numOfSwitch; j++) {
                tmpLink = {j, 0};
                statInfo.push_back(tmpLink);
            }
        } */  
        statInfo = newWeight[i];
        stat_packet_event(i, con_id, statInfo, UpdTime, "stat");
    }
